...
while ($line = db_read($rlt))
	...
	if ($em == '1')
		...
		echo numberformat_br($parcelas,0).'x';
		ECHO '<td align="right">';
		echo numberformat_br($anuidade,2);
...
//		echo $xsql;
		...
		array_push($iisql,"update contrato set ctr_anuidade_atual = ".$anuidade.", ctr_reajuste = '".date("Ymd")."' where id_ctr = ".$idc.';'.chr(13));
		while (round($saldo) > 1)
			{
	...
	echo '<TR><TD colspan="10" align="right">'.numberformat_br($tt1,2).'</TD></TR>';
	ECHO '</table>';
	if (count($iisql) > 0)
	{
		for ($r=0;$r < count($iisql);$r++)
			{
				//echo $iisql[$r];
				//echo '<HR>';
				$rrr = db_query($iisql[$r]);
			}
		//echo mst($isql);
