...
////////////////////////////// Boletos Quitados
//$sql = "update cr_boleto set bol_status='A' where (bol_status isnull) or (length(trim(bol_status))=0) ";
//$rlt = db_query($sql);
...
$sql = "select * from cr_boleto where bol_status = 'A' order by bol_data_vencimento";
$rlt = db_query($sql);

while ($line = db_read($rlt))
	{
	$valor = $line['bol_valor_boleto'];
	$dt = $line['bol_data_vencimento'];
...
//	echo '<BR>'.$dt_ano.'=='.$dt_mes.'=='.$valor;
	...
	for ($kk=0;$kk < 5;$kk++)
		...
		for ($ky=0;$ky < 12;$ky++)
			{
			$link = '<A HREF="boleto_detalhe.php?dd1='.($ky+1).'&dd2='.(date("Y")-$kk).'&dd3=A">';
	...
	echo gr_resumo($rsm,$resumo,'Resumo boletos abertos');
	echo '<HR width="400">';
	echo gr_barras($vv,'Boletos abertos '.(intval(date("Y"))),220);
	echo '<HR width="400">';
	echo gr_barras($vv_lastyear,'Boletos abertos '.(intval(date("Y")-1)),220);
///////////////////////////////////////////////////////////////
require("foot.php");	
?>