...
function agenda($data,$dias)
{
	...
	for ($kk=0;$kk < count($dias);$kk++)
		...
		$link = '<A HREF="#" onclick="newxy2('."'cryo_calendario_novo.php?dd0=".$dias[$kk][5]."'".',600,500);">';
		$ss.= '<TR '.$xcor.'>';
		$ss.= '<TD>';
		$ss.= $link;
		$ss.= $dias[$kk][0];
...
function calendario($data,$dias)
{
	...
	if (strlen($data) == 6)
	{
		...
		$ddx=1;
		$ydata = xstod($ano.$mes.'01');
		$xdata = xstod($ano.$mes.'01');
		$ys = Date('w',$ydata);
		$ss.='<TR align="center">';
...
//				echo $dias[$ky][0].'='.$ddx.'<BR>';
			...
			$xdata = xstod($ano.$mes.strzero($ddx,2));
			$bold = '';
			if (date("m",$xdata) != date("m",$ydata)) { $kk=99; }
				else
				{
					if (strlen($xlink) ==0) 
						{
							$link = '<A HREF="#" onclick="newxy2('."'cryo_calendario_novo.php?dd1=".date("d/m/Y",$xdata)."'".',600,500);">';
						} else {
