...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
    ...
    if( $use_swfobject )
    {
		...
		$out[] = 'var so = new SWFObject("'. $base .'sisdoc_flash-chart.swf", "'. $obj_id .'", "'. $width . '", "' . $height . '", "9", "#FFFFFF");';
    ...
    $out[] = '<param name="movie" value="'. $base .'sisdoc_flash-chart.swf?data='. $url .'" />';
    $out[] = '<param name="quality" value="high" />';
    $out[] = '<param name="bgcolor" value="#FFFFFF" />';
    $out[] = '<embed src="'. $base .'sisdoc_flash-chart.swf?data=' . $url .'" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
