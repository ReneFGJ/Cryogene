...
class email
	{
		...
		function resumo_email_enviar()
			{
				$sql = "select count(*) as total from _email where ma_status = 'A' limit 1";
				$rlt = db_query($sql);
				$line = db_read($rlt);
				$total = $line['total'];
				return($total);				
		...
		function enviar_proximo()
			{
				$sql = "select * from _email where ma_status = 'A' order by id_ma
						limit 1";
				$rlt = db_query($sql);
				if ($line = db_read($rlt))
					{
					...
					$texto = $line['ma_texto'];
					$head = $line['ma_hed'];
					mail($email,$sub,$texto,$head);
					$sql = "update _email set ma_status = 'B',
						ma_enviado = ".date("Ymd").",
						ma_enviado_hora = '".date("H:i:s")."'  
						where id_ma = ".$id;
					$rlt = db_query($sql);
...
//	$body = '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">'."\n".$body;
	...
	if (mail_grava($to, $subject, $body, $headers)) {
	   return('OK');
	 } else {
	  return('ERRO');
	  }
...
function mail_grava($to,$sub,$body,$hed)
	{
		$data = date("Ymd");
		$hora = date("H:m:i");
		$ip = $_SERVER['REMOTE_ADDR'];
		$sql = "insert into _email 
			(ma_email,ma_assunto,ma_texto,ma_hed,ma_status,ma_data,ma_hora,ma_enviado,ma_enviado_hora,ma_ip)
			values
			('$to','$sub','$body','$hed','A',$data,'$hora',19000101,'','$ip');";
		$rlt = db_query($sql);
	}
