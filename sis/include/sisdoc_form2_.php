...
// Versão atual           //    data     //
//---------------------------------------//
// 0.0g                       20/01/2009 // COrrecao do Chechar CPF  e Implantado HV (Hidden com valor)
// 0.0f                       06/07/2008 // funcao Checar CPF 
// 0.0e                       06/07/2008 // funcao [de-ate] e [Data=8]
// 0.0d                       24/06/2008 //
// 0.0c                       15/05/2008 //
// 0.0b                       21/04/2008 //
// 0.0a                       20/05/2008 //
///////////////////////////////////////////
if ($mostar_versao == True) { array_push($sis_versao,array("sisDOC (Form2)",'0.0f',20080728)); }
global $form;
//if ($form != true)
{
$form = True;
...
function gets($c1,$txt,$c3,$c4,$c5,$c6)
	{
	global $estilo,$acao,$editor,$ed_nr_id,$script;
...
//		$gets = $gets . "<TD>";
//		$gets = $gets . "<input type=\"text\" name=\"".$c1."\" value=\"".$txt."\" size=\"".$size."\" maxlength=\"".$mvl."\" ".$ce."></TD>";
		$msk = 'onkeypress="return sonumero(event,this.id);" style="text-align: right;" '.$ce;
...
//		$gets = $gets . "<TD>";
//		$gets = $gets . "<input type=\"text\" name=\"".$c1."\" value=\"".$txt."\" size=\"".$size."\" maxlength=\"".$mvl."\" ".$ce."></TD>";
		$msk = 'onkeypress="return(currencyFormat(this,event));return sonumero(event,this.id);" style="text-align: right;" '.$ce;
		$gets = $gets. '<TD><input type="text" name="'.$c1.'" id="'.$c1.'"  value="'.$txt.'" size="'.$size.'" maxlength="'.$mvl.'" '.$msk.'></TD>';
		}
...
//************************************************* STRING
	if (substr($c3,1,2)=="UF")
		...
		$cmd='#';
		$gets = "";
		$ufs = array("AL", "AM", "AP", "BA", "CE", "DF", "ES", "GO", "MA", "MG", "MS", "MT", "PA", "PB", "PE", "PI", "PR", "RJ", "RN", "RR", "RS", "SC", "SE", "SP", "TO");
		$gets = "";
		if (strlen($c4) > 0) 
			{ $gets = $gets . "<TD align=\"right\" valign=\"top\">".$cf.$c4."</TD>"; }
		$gets .= "<TD>";
