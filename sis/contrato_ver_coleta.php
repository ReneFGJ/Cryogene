...
if (!($line = db_read($rlt)))
	{
		...
		$sxr = $sxr . '<TD>';
			...
			require("contrato_documentos.php");
		...
		echo '<TD>';
		echo '<form method="post" action="contrato_processamento.php">';
		echo '<input type="hidden" name="dd0" value="'.$idproc.'">';
		echo '<input type="hidden" name="dd1" value="'.$contrato.'">';
		echo '<input type="submit" name="botao" value="processamento">';
		echo '</form>';
		echo '</TD>';

		echo '<TD>';
		echo '<form method="post" action="contrato_processamento_verso.php">';
		echo '<input type="hidden" name="dd0" value="'.$contrato.'">';
		echo '<input type="submit" name="botao" value="processamento (verso)">';
		echo '</form>';
		echo '</TD>';
		?>
		<TR><TD></TD><TD></TD>
		<TD>
		<? //if (strlen($idcong) == 0) { ?>
		<input type="button" name="botao" value="congelamento" onclick="newxy('contrato_congelamento.php?dd0=<?=$idcong;?>&dd1=<?=$contrato;?>',710,400);">
		<? //} ?>
		</TD>
		<TD>
		<? //if (strlen($idarma) == 0) { ?>
		<input type="button" name="botao" value="armazenamento" onclick="newxy('contrato_armazenamento.php?dd0=<?=$idarma;?>&dd1=<?=$contrato;?>',710,400);">
		<? //} ?>
		</TD>						
		<?
		echo '</table>';
