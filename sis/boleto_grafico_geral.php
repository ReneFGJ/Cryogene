...
////////////////////////////// Boletos Quitados
$sql = "update cr_boleto set bol_status='A' where bol_status ='' ";
$rlt = db_query($sql);
...
$sql = "select * from cr_boleto where (bol_status = 'B' or bol_status = 'A') order by bol_data_vencimento";
$rlt = db_query($sql);
...
while ($line = db_read($rlt))
	...
	$sta = $line['bol_status'];
	if ($sta == 'B')
		{ $valor = $line['bol_valor_pago']; 	$dt = $line['bol_data_pago']; }
	else { $valor = $line['bol_valor_boleto']; 	$dt = $line['bol_data_vencimento']; }
...
//	echo '<BR>'.$dt_ano.'=='.$dt_mes.'=='.$valor;
	...
	for ($kk=0;$kk < 5;$kk++)
		...
		for ($ky=0;$ky < 12;$ky++)
			{
			$link = '<A HREF="boleto_detalhe.php?dd1='.($ky+1).'&dd2='.(date("Y")-$kk).'&dd3=*">';
	...
	echo gr_resumo($rsm,$resumo,'Resumo anual de receita / Abertos-Quitados<BR>(por vencimento)');
	echo '<HR width="400">';
