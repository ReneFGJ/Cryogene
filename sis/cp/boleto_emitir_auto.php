...
$sql = "update contrato set ctr_reajuste_indice = -1.72";
$rlt = db_query($sql);
...
if (strlen($dd[0]) == 0)
	...
	$sql .= ' order by ctr_dt_assinatura,ctr_dt_assinatura,ctr_numero ';
	$rlt = db_query($sql);
	/////////////////////////////////////////////// GERAR BOLETOS
	...
	while ($line = db_read($rlt))
		...
		if ($ano != $xano)
			...
			//if ($btot1 > 0)
				...
				echo 'Total do ano <B>'.number_format($btot1,2);
				echo '</TD></TR>';
				$btot1=0;
				}
		...
		//////////////////////////////////////////////////////////////
		if ($anuidade > 0)
			{ $dsp_anuidade = number_format($anuidade,2); } else
			{ 
			if ($anuidade == 0)
		...
		if ($parcelas > 0)
			{
			$par = number_format(intval(10 * $anuidade / $parcelas)/10,2);
		...
		echo '<TD align="center">'.stodbr($data_ass);
		...
		echo '<TD colspan="2">';
		echo '['.number_format($boleto,2).']';
		echo '<TD>';
		echo '('.number_format($indice,2).')';
		...
		echo '<TR><TD colspan="10"><B>Total de '.$tot.' contrato, R$ '.number_format($tov,2);
		if ($toit > 0)
