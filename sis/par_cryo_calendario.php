...
function agenda($data,$dias)
{
	...
	for ($kk=0;$kk < count($dias);$kk++)
		...
		$link = '<A HREF="#" onclick="newxy('."'par_cryo_calendario_novo.php?dd0=".$dias[$kk][5]."'".',550,300);">';
		$ss.= '<TR '.$xcor.'>';
		$ss.= '<TD>';
		$ss.= $link.'[1]';
		$ss.= $dias[$kk][0];
		...
		for ($kk=0;$kk<40;$kk++)
			...
			if (date("m",$xdata) != date("m",$ydata)) { $kk=99; }
				else
				{
					if (strlen($xlink) ==0) 
						{
							$link = '<A HREF="#" onclick="newxy('."'par_cryo_calendario_novo.php?dd1=".date("d/m/Y",$xdata)."'".',550,300);">';
						} else {
